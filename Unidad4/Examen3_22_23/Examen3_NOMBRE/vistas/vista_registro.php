...
// vista del registro para insertar clientes

// si le ha dado a volver te lleva al index
if (isset($_POST["btnVolver"])) {
    header("Location: index.php");
    exit();
}

// si continua con el registro
if (isset($_POST["btnContRegistro"])) {

    // si mo hay error de usuario
    $error_usuario = $_POST["usuario"] == "" || strlen($_POST["usuario"]) > 20;
    if (!$error_usuario) {
        // si no existe la conexion hace otra
        if (!isset($conexion)) {
            try {
    ...
    // si no hay error de clave ni foto
    ...
    if (!$error_form) {
        ...
        // guarda los mismos datos que al hacer el login
...
<body>
    ...
    <form action="index.php" method="post" enctype="multipart/form-data">
        <!-- formulario con todo lo necesario para meter a un cliente -->
        <p>
